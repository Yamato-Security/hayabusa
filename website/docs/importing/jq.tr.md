...
Hayabusa'nın mevcut sürümünde, sonuçları JSON olarak `hayabusa dfir-timeline -t json -d <directory> -o results.json` ile veya JSONL biçimi için `hayabusa dfir-timeline -t jsonl -d <directory> -o results.jsonl` ile kaydedebilirsiniz.
