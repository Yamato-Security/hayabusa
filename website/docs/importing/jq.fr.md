...
Dans la version actuelle de Hayabusa, vous pouvez enregistrer les résultats au format JSON avec `hayabusa dfir-timeline -t json -d <directory> -o results.json` ou `hayabusa dfir-timeline -t jsonl -d <directory> -o results.jsonl` pour le format JSONL.
