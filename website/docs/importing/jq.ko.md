...
현재 버전의 Hayabusa에서는 `hayabusa dfir-timeline -t json -d <directory> -o results.json`으로 결과를 JSON으로 저장하거나, JSONL 형식의 경우 `hayabusa dfir-timeline -t jsonl -d <directory> -o results.jsonl`로 저장할 수 있습니다.
