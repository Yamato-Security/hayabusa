...
現在のバージョンのHayabusaでは、`hayabusa dfir-timeline -t json -d <directory> -o results.json`でJSON、`hayabusa dfir-timeline -t jsonl -d <directory> -o results.jsonl` でJSONLに結果を保存できます。
