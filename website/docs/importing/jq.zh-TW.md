...
在目前的 Hayabusa 2.x 版本中，你可以用 `hayabusa json-timeline -d <directory> -o results.json` 將結果儲存為 JSON，或用 `hayabusa json-timeline -d <directory> -J -o results.jsonl` 儲存為 JSONL 格式。
