...
為了節省空間，我們會縮寫等級、MITRE ATT&CK 戰術、頻道、提供者、欄位名稱等等……

您可以使用 `-b, --disable-abbreviations` 選項關閉其中部分縮寫，以檢視原始的頻道名稱、提供者名稱等等……

## 等級縮寫

為了節省空間，在顯示警示 `level` 時，我們使用以下縮寫。
...
為了節省空間，在顯示 MITRE ATT&CK 戰術標籤時，我們使用以下縮寫。
您可以在 `./config/mitre_tactics.txt` 設定檔中自由編輯這些縮寫。
...
為了節省空間，在顯示頻道時，我們使用以下縮寫。
您可以在 `./rules/config/channel_abbreviations.txt` 設定檔中自由編輯這些縮寫。
