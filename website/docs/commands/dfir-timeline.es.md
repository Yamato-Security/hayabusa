...
Los comandos `csv-timeline` y `json-timeline` ahora tienen un asistente de escaneo habilitado de forma predeterminada.
...
Si desea usar estas dos reglas y escanear todas las reglas contra los archivos `.evtx` cargados, entonces necesitará añadir la opción `-A, --enable-all-rules` en los comandos `csv-timeline` y `json-timeline`.
...
Si está seguro de que se están usando múltiples canales dentro de un solo archivo `.evtx`, por ejemplo si alguien usó una herramienta para fusionar múltiples archivos `.evtx`, entonces puede desactivar este filtrado con la opción `-a, --scan-all-evtx-files` en los comandos `csv-timeline` y `json-timeline`.
...
El comando `csv-timeline` creará una cronología forense de eventos en formato CSV.
...
### Ejemplos del comando `csv-timeline`
...
* Cuando se usa `csv-timeline`, se mostrarán adicionalmente las siguientes 6 columnas: `SrcASN`, `SrcCity`, `SrcCountry`, `TgtASN`, `TgtCity`, `TgtCountry`.
* Cuando se usa `json-timeline`, los mismos campos `SrcASN`, `SrcCity`, `SrcCountry`, `TgtASN`, `TgtCity`, `TgtCountry` se añadirán al objeto `Details`, pero solo si contienen información.
...
El comando `json-timeline` creará una cronología forense de eventos en formato JSON o JSONL.
Generar la salida a JSONL será más rápido y de menor tamaño de archivo que JSON, por lo que es bueno si solo va a importar los resultados a otra herramienta como Elastic Stack.
JSON es mejor si va a analizar manualmente los resultados con un editor de texto.
La salida CSV es buena para importar cronologías más pequeñas (normalmente menos de 2GB) a herramientas como LibreOffice o Timeline Explorer.
JSON es mejor para un análisis más detallado de los datos (incluyendo archivos de resultados grandes) con herramientas como `jq`, ya que los campos `Details` están separados para facilitar el análisis.
(En la salida CSV, todos los campos del registro de eventos están en una gran columna `Details`, lo que dificulta la ordenación de datos, etc...)
...
### Ejemplos del comando `json-timeline` y archivos de configuración

Las opciones y archivos de configuración para `json-timeline` son los mismos que para `csv-timeline` pero con una opción adicional `-L, --JSONL-output` para generar la salida en formato JSONL.
