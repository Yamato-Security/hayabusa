...
- Fixed `log-metrics` skewing the First/Last Timestamp columns for Splunk-JSON input: the timestamp fallback parsed the explicit UTC offset (e.g. `+09:00`) with `NaiveDateTime`, which discards the offset, storing the local wall-clock time as if it were UTC. The offset is now applied. (#1897) (@YamatoSecurity)
