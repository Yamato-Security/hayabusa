...
- Renamed 74 cryptic or hayabusa-specific variable, parameter, and struct-field names in `src/` (e.g. `datas`→`records`, `con_cal`→`joined_value`, `name_2_node`→`name_to_node`, `hlch`→`horizontal_line_char`, `rulepath`→`rule_path`, `ext_field`→`output_fields`) to clearer, idiomatic Rust names for readability. Internal identifiers only; no behavior or output changes. (#1830) (@YamatoSecurity)
