...
- Split `src/detections/rule/mod.rs` (~1,200 lines that mixed the module declarations with the core rule types and a large test module) into a new `rulenode.rs` holding `RuleNode`, `DetectionNode`, `AggResult`, `CorrelationType`, `create_rule`/`get_detection_keys`, and the tests; `mod.rs` is now just module wiring plus `pub use rulenode::*;` so every existing import path is unchanged. Pure code movement with byte-identical detection output. (#1890) (@YamatoSecurity)
