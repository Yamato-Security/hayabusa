...
## 文字列長比較により、フィルターする
扱う文字列の特性に依っては、簡単なフィルターをすることで、文字列マッチング回数を減らすことで高速化できる場合があります。
...
### 効果（Pull Reuest事例）   <!-- omit in toc -->
[Hayabusa](https://github.com/Yamato-Security/hayabusa)では、大文字小文字を区別しない文字列比較をする必要があるため、[to_lowercase()](https://doc.rust-lang.org/std/string/struct.String.html#method.to_lowercase)を実施したうえで、上記を適用しました。その場合でも以下PRの事例では、
