...
- JSON/JSONL/Splunk入力のスキャン時に、タイムスタンプ解析エラーの警告メッセージが`--verbose`/`--quiet-errors`オプションに関わらず予期せず表示され、エラーログにも記録されない問題を修正した。EVTXレコード解析パスと同様にゲートされるようになった。 (#1762) (@YamatoSecurity)
