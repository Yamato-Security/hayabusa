...
- csv-timeline` と `json-timeline` コマンドに `--include-category` と `--exclude-category` オプションを追加した。include-category` は指定された `category` フィールドでのみスキャンを許可する。cv-timeline` と `json-timeline` コマンドのオプション `--exclude-category` は指定した `category` フィールドを持つルールのスキャンを除外する。 (#1119) (@hitenkoku)
