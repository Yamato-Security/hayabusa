...
- `log-metrics` が Splunk-JSON 入力の First/Last Timestamp 列をずらしていた問題を修正した。タイムスタンプのフォールバックが明示的な UTC オフセット（例: `+09:00`）を `NaiveDateTime` で解析していたためオフセットが破棄され、ローカルの時計時刻がそのまま UTC として保存されていた。オフセットを適用するようにした。 (#1897) (@YamatoSecurity)
