...
You can use the `eid-metrics` command to print out the total number and percentage of event IDs (`<System><EventID>` field) separated by channels.
...
However, sometimes risk levels are not consistent and also the actual risk level may differ according to your environment.
...
We have created a separate project to document what log settings need to be enabled as well as scripts to automatically enable the proper settings at [https://github.com/Yamato-Security/EnableWindowsLogSettings](https://github.com/Yamato-Security/EnableWindowsLogSettings).
