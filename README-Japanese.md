...
注意: アップデートを実行する際に `rules` フォルダ内のレポジトリに`git reset --hard HEAD`が実行され、強制的にgitで管理しているバージョンに変更されます。
もし、`--level-tuning`以外で既存のルールを修正している場合は、`rules`フォルダ以外にバックアップを取ることをおすすめします。
